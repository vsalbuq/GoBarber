...
class AppointmentController {
  ...
  async store(req, res) {
    ...
    /**
     * Check if provider is creating an appointment with himself
     */
  ...
  async delete(req, res) {
    ...
    if (isBefore(subDate, new Date())) {
      return res.status(401).json({
        error: 'Appointments can be cancelled only until 2 hours before it.',
      });
    }
    ...
    await Mail.sendMail({
      ...
      context: {
        ...
        date: format(appointment.date, "dd MMMM', 'H:mm'h'"),
      },
    });
